...
#define MAX_LABELS 300 // Define maximum number of labels
...
void execute_instruction(const char *instruction, int line) {
    ...
    // Execute the corresponding operation based on the mnemonic
    if (strcmp(op, "ADD:") == 0) {
        arm_regs[dest_reg] = ADD(op1_val, op2_val); // Execute ADD operation
        printf("  result = %d \n", arm_regs[dest_reg]); // Print result
    } else if (strcmp(op, "SUB:") == 0) {
    arm_regs[dest_reg] = SUB(op1_val, op2_val); // Execute SUB operation
    printf("  result = %d \n", arm_regs[dest_reg]); // Print result
} else if (strcmp(op, "MUL:") == 0) {
    arm_regs[dest_reg] = MUL(op1_val, op2_val); // Execute MUL operation
    printf("  result = %d \n", arm_regs[dest_reg]); // Print result
} else if (strcmp(op, "DIV:") == 0) {
    if (op2_val != 0) {  // Check for division by zero
        arm_regs[dest_reg] = DIV(op1_val, op2_val); // Execute DIV operation if divisor is not zero
        printf("  result = %d \n", arm_regs[dest_reg]); // Print result
    } else {
    ...
    sscanf(instruction, "JE: %s", target_label); // Extract target label
    if (flags == 0) { // Check if the zero flag is set
        for (int i = 0; i < label_count; i++) {
            if (strcmp(labels[i].name, target_label) == 0) {
                // Found the target label
                arm_regs[R_PC] = labels[i].lines_number; // Jump to the target label
                return;
            }
        }
...
int main() {
    ...
    uint16_t current_line = 0; // Initialize current line number
    for (int i = 0; i < 16; i++) {
    ...
    printf("Number of lines executed: %d\n", current_line); // Print the number of lines executed
