...
void execute_instruction(const char *instruction, int line)
{
    ...
    else if (strcmp(op, "CALL:") == 0)
    {
        ...
        for (int i = 0; i < label_count; i++)
        {
            if (strcmp(labels[i].name, target_label) == 0)
            {
                ...
                arm_regs[R_LR] = arm_regs[R_PC] + 1;     // Store return address
                arm_regs[R_PC] = labels[i].lines_number; // Jump to target
                return;
            }
        }
    ...
    {
        if (arm_regs[R_LR] != 0xFFFF)
        {
            arm_regs[R_PC] = arm_regs[R_LR]; // Jump back to return address
            arm_regs[R_LR] = 0xFFFF;             // Clear Link Register
            return;
        }
        else
        {
            printf("Error: No return address\n");
            exit(1);
        }
    }
