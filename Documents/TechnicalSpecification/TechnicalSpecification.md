...
- [2.Project infrastructure](#2project-infrastructure)
  ...
  - [2.2 Assembly language design](#22-assembly-language-design)
    - [2.2.1 Storing an immediate value storage into a register](#221-storing-an-immediate-value-storage-into-a-register)
    - [2.2.2 Register-to-register value copying](#222-register-to-register-value-copying)
    - [2.2.3 Memory value read from the address stored in a register](#223-memory-value-read-from-the-address-stored-in-a-register)
    - [2.2.4 Register value stored into memory at the address contained in another register](#224-register-value-stored-into-memory-at-the-address-contained-in-another-register)
    - [2.2.4 Comparison of register contents](#224-comparison-of-register-contents)
    - [2.2.5 Unconditional jump to a label](#225-unconditional-jump-to-a-label)
    - [2.2.6 Conditional jump instructions](#226-conditional-jump-instructions)
    - [2.2.7 Subroutine call and return:](#227-subroutine-call-and-return)
    - [2.2.8 Basic Arithmetic Operations: Addition, Subtraction, Multiplication, and Division:](#228-basic-arithmetic-operations-addition-subtraction-multiplication-and-division)
    - [2.2.9 Basic Logical Operations: OR, AND, XOR, and NOT](#229-basic-logical-operations-or-and-xor-and-not)
- [3.Architecture](#3architecture)
  ...
  - [3.3 Software](#33-software)
    - [3.3.1 ``workspace.xml``](#331-workspacexml)
    - [3.1.2 ``gate_operation.h``](#312-gate_operationh)
    - [3.1.3 ```mnemonics.h```](#313-mnemonicsh)
    - [3.1.2 ```operation.h```](#312-operationh)
 ...
 */
 ...
 ## 2.2 Assembly language design
 
 ### 2.2.1 Storing an immediate value storage into a register
...
### 2.2.2 Register-to-register value copying
...
### 2.2.3 Memory value read from the address stored in a register
...
### 2.2.4 Register value stored into memory at the address contained in another register
...
### 2.2.4 Comparison of register contents 
...
### 2.2.5 Unconditional jump to a label
...
### 2.2.6 Conditional jump instructions
...
### 2.2.7 Subroutine call and return:
...
### 2.2.8 Basic Arithmetic Operations: Addition, Subtraction, Multiplication, and Division:
...
### 2.2.9 Basic Logical Operations: OR, AND, XOR, and NOT
...
OR: R10 R3 R4  ; Perform bitwise OR operation between registers R3 and R4, store result in R10

XOR: R11 R1 R2  ; Perform bitwise XOR operation between registers R1 and R2, store result in R11

NOT:  R12 R3      ; Perform bitwise NOT operation on register R3 and store the result in R12
```

**Explanation** :
- AND Operation (``AND``): Bitwise AND between the contents of registers R1 and R2. The result is stored in register R9.
  
- OR Operation (``OR``): Bitwise OR between the contents of registers R3 and R4. The result is stored in register R10.
  
- XOR Operation (``XOR``): Bitwise XOR between the contents of registers R1 and R2. The result is stored in register R11.
  
- NOT Operation (``NOT ``): Bitwise NOT on the contents of register R3. The result is stored in register R12.
...
- OR Operation (``OR``): Sets each bit to 1 if at least one corresponding bit of the operands is 1.
  
- XOR Operation (``XOR``): Sets each bit to 1 if the corresponding bits of the operands are different.
  
- NOT Operation (``NOT ``): Flips each bit, changing 1s to 0s and vice versa
  
<br>


__Among all these mnemonics, our team decided to select 16 of these for this project, which are :__
...
By selecting these 16 mnemonics for our project, we have created a solid foundation for the implementation of our processor ritual. These essential instructions will enable us to develop a robust and functional system that meets our requirements.

<br>



__And we used no fewer than 15 registers :__
...
### 3.3.1 ``workspace.xml``
We decided to create a XML (eXtensible Markup Language) to store project's specific configurations and parameter that we called ```workspace.xml``` here's what it contains :

- Auto import settings: Configuration for auto-import settings, specifying selective auto-reload.

- C make settings: Configuration for CMake settings, defining a "Debug" configuration.

- Change list manager: Manages changes in the project, listing modifications made to specific files.

- Clangd settings: Settings related to the Clangd language server.

- Git.settings: Git settings, including the recent Git root path.

- Markdown settings migration: Migration settings related to Markdown settings.

- Project Id: Unique identifier for the project.

- Project View State: Configuration related to the project view state.

- Spell checker settings: Configuration related to the spell checker settings.

- Task manager: Manages tasks within the project.

### 3.1.2 ``gate_operation.h``

This file define various operations and data structures for the virtual processor :

![imagesdiagram](./images/diagram.png)

In this diagram:

- ``gate_operation.h`` is the header file being represented.

- Components such as Memory, Registers, Operations, and Enums are encapsulated within the header file.
  
- Memory component includes the memory array and the ``mem_read()`` function.

- Registers component includes the arm_regs array and the ``initialize_arm_regs()`` function.
  
- Operations component includes logical operations functions like ``AND()``, ``OR()``, ``NOT()``, and ``XOR()``.
  
- Enums component includes the registers enumeration defining symbolic names for registers.

### 3.1.3 ```mnemonics.h```

- This file contains an enumeration mnemonics defining symbolic names for different instructions.
  
- It also declares variables like flags and functions for various instructions.

- It encapsulates mnemonic definitions and associated functions, allowing other parts of the codebase to use mnemonic instructions without redefining or duplicating them.

### 3.1.2 ```operation.h```

- This file contains an enumeration operation defining symbolic names for different arithmetic operations.

- It declares a variable instruct.
  
- It declares functions for various arithmetic operations like ADD(), SUB(), etc.

- This header file encapsulates arithmetic operation definitions, allowing other parts of the codebase to use these operations without redefining or duplicating them.



## 4.Conclusion

In conclusion The technical specifications outline the development process for the virtual processor project. Beginning with an introduction providing clarity on document objectives and scope, the specifications proceed to define implementation requirements, including hardware and software prerequisites, setup, and execution instructions.

The project infrastructure section establishes conventions for directory structure, naming, and formatting, ensuring consistency throughout development. A detailed explanation of the assembly language design follows, covering various instructions for register manipulation, memory access, control flow, and arithmetic/logical operations.

The architecture section delves into the interpreter and assembler components. The interpreter's error handling mechanism is outlined, including technical execution processes, error detection, output handling, and examples. Similarly, the assembler's functionality is detailed, encompassing parsing, symbol table management, translation, error handling, and optional optimization techniques.

Finally, the specifications conclude with a section on software, mentioning the use of XML, presumably for configuration or other purposes. This comprehensive document serves as a roadmap for the development of the virtual processor project, ensuring clarity, consistency, and functionality across all aspects of implementation.